...
public class UserGetHelper
{
	 ...
	 * @throws AidException
	 */
	public UserGetResponseBean getUsersInfo(UserGetRequestParam param)
			throws AidException, Throwable
	{
		Bundle parameters = param.getParams();
		UserGetResponseBean usersBean = null;
		try
		{
			...
			usersBean = new UserGetResponseBean(response);
		}
			...
			final AbstractRequestListener<UserGetResponseBean> listener)
	{
		pool.execute(new Runnable() {
			...
			public void run()
			{
				try
				{
					UserGetResponseBean usersBean = getUsersInfo(param);
					if (listener != null)
					{
